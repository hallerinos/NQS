...
if __name__ == "__main__":
    ...
    with Profiler(interval=0.1) as profiler:
        for epoch in epochbar:
            block.bsample_block(wf, n_res=n_spins)

            Eav = torch.mean(block.EL, dim=0)
            Oav = torch.mean(block.OK, dim=0)
            epsbar = (block.EL - Eav) / n_block**0.5
            Obar = (block.OK - Oav) / n_block**0.5


            if wf.n_param < n_block:
                metric_tensor = Obar.conj().T @ Obar
                metric_tensor = metric_tensor + 1e-4*torch.eye(metric_tensor.shape[0], device=wf.device, dtype=wf.dtype)
                dEdTh = Obar.conj().T @ epsbar
                dThn = torch.linalg.solve(metric_tensor, dEdTh)
            else:
                kernel = Obar @ Obar.conj().T
                kernel = kernel + 1e-4*torch.eye(kernel.shape[0], device=wf.device, dtype=wf.dtype)
                b = torch.linalg.solve(kernel, epsbar)
                dThn = Obar.conj().T @ b
