...
if __name__ == "__main__":    
    ...
    Ok = torch.where(abs(Ok) < 2, 0, Ok).to_sparse()
    # print(Ok)
    Amat = (Ok.T.conj() @ Ok).to_sparse()
    Amat = Amat + 1e-8*torch.eye(Amat.shape[0], dtype=Ok.dtype).to_sparse()
    # Amat = Amat + Amat.T.conj()
    b = torch.randn((Amat.shape[1],1), dtype=Amat.dtype).to_sparse()
