...
from ansatz.CNN import FFNN
from vmc.iterator_nn import MCBlock
from icecream import ic
from pyinstrument import Profiler
from vmc.minSR import minSR
from vmc.SR import SR, SR_, SR__
import numpy as np
from energies.TFIM_nn import local_energy
# torch.manual_seed(0)
...
if __name__ == "__main__":
    ...
    dTh = torch.zeros(wf.n_param).to(device, dtype)
    with Profiler(interval=0.1) as profiler:
        for epoch in epochbar:
            ...
            f = lambda primals: torch.func.functional_call(wf, primals, block.samples)
            vjpout = torch.func.vjp(f, wf.state_dict())
            vjpf_out = vjpout[1](block.EL)[0]

            fav = lambda primals: torch.func.functional_call(wf, primals, block.samples).mean()
            vjpout = torch.func.vjp(fav, wf.state_dict())
            vjpfav_out = vjpout[1](Eav)[0]

            dTh = vjpf_out
            for (i1, i2) in zip(dTh.items(), vjpfav_out.items()):
                i1[1].mul_(-eta/n_block)
                i1[1].add_(-eta*i2[1])
