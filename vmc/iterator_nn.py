import torch
from sampler.mcmc_nn import draw_next
...
class MCBlock:
    def __init__(self, wf, n_block, local_energy=lambda x, y: 1):
        ...
        self.samples = (2.0*torch.randint(0, 2, (n_block, wf.n_spins)) - 1.0).to(next(wf.parameters()).device, next(wf.parameters()).dtype)
        self.EL = torch.zeros(n_block).to(self.samples.device, self.samples.dtype)
